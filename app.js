"use strict";
/* ═══════════════════════════════════════════════════════════════════════════
   AGATHE — SUIVI BÉBÉ · app.js
   ───────────────────────────────────────────────────────────────────────────
   NAVIGATION : recherchez (Ctrl+F / Cmd+F) le repère entre crochets,
   par ex. « [10] », pour sauter directement à une section.

   ┌──── SOMMAIRE ───────────────────────────────────────────────────────────┐
   │ [1]  ÉTAT & PERSISTANCE     état, sauvegarde locale, accroche de partage  │
   │ [2]  UTILITAIRES            $, $$, dates, heures, échappement HTML        │
   │ [3]  ÂGE DU BÉBÉ            ageText()                                     │
   │ [4]  CONSTANTES D'AFFICHAGE courbes OMS, couleurs/icônes d'événements     │
   │ [5]  NAVIGATION & THÈME     onglets, mode nuit                           │
   │ [6]  BIBERONS — PLANNING    planForToday, baseGrid, nextSlot, renderPlan  │
   │ [7]  BIBERONS — SAISIE      par qui, tirage, validation d'une prise       │
   │ [8]  SOINS                   compteurs bain / pipi / selle                │
   │ [9]  PHYSIONOMIE & COURBE   poids/taille/PC + courbe de croissance        │
   │ [10] TRAITEMENTS            liste, ajout, « donné aujourd'hui »           │
   │ [11] PLANNING / CALENDRIER  calendrier + rendez-vous + to-do list           │
   │ [12] CONTACTS / URGENCE     pédiatre, sage-femme                          │
   │ [13] RÉGLAGES               prénom, planning, rappel, partage             │
   │ [14] RAPPEL & BERCEUSE      notification + petite musique                 │
   │ [15] EXPORT PDF             récapitulatif semaine / mois                  │
   │ [16] RENDU GLOBAL & INIT    rerenderAll, AgatheApp, démarrage             │
   │ [17] SERVICE WORKER         installation / hors-ligne                     │
   └──────────────────────────────────────────────────────────────────────────┘
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════════════
   [1] ÉTAT & PERSISTANCE
   - KEY / DEFAULTS : clé de stockage et état par défaut
   - clone / deepMerge / load : chargement de l'état
   - save() : enregistre en local ET déclenche le partage (share.js)
   ═══════════════════════════════════════════════════════════════════════════ */
const KEY = "agathe.v1";
...
const DEFAULTS = {
  ...
  diapers: {},      // { "YYYY-MM-DD": {bain,pipi,caca} }
  ...
  todos: []         // [ {id,text,done} ]  — liste "À faire"
};
...
// Enregistre l'état. L'accroche window.Sync (définie par share.js) propage
// automatiquement la modification vers GitHub si le partage est activé.
function save() {
  localStorage.setItem(KEY, JSON.stringify(state));
  if (window.Sync && Sync.onLocalSave) Sync.onLocalSave(state);
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   [4] CONSTANTES D'AFFICHAGE
   - WHO_* : valeurs indicatives (type OMS, filles) pour la courbe de poids
   - EVT_COLOR / EVT_ICON : couleurs et icônes des types de rendez-vous
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════════════
   [6] BIBERONS — PLANNING
   - planForToday() : prises déjà faites + créneaux à venir (recalage possible)
   - baseGrid()     : grille fixe de référence
   - nextSlot()     : prochain biberon prévu
   - renderPlan()   : affichage ; renderFeedSummary() : total du jour
   - tickTime()     : horloge auto du formulaire de prise
   ═══════════════════════════════════════════════════════════════════════════ */
function planForToday() {
  const d = dateKey();
  const s = state.settings;
  const orig = state.feeds[d] || [];
  const feeds = orig.map((f, idx) => ({ f, idx })).sort((a, b) => toMin(a.f.time) - toMin(b.f.time));
  const slots = [];
  feeds.forEach(({ f, idx }) => slots.push({
    time: f.time, who: f.who, vol: num(f.maternel) + num(f.gallia), done: true, tirage: !!f.tirage, idx: idx
  }));

  let who, anchor;
  if (feeds.length && s.recal) {
    who = other(feeds[feeds.length - 1].f.who);
    anchor = toMin(feeds[feeds.length - 1].f.time) + s.intervalH * 60;
  } else if (feeds.length) {
    // grille fixe : prochain créneau de la grille après la dernière prise
    const grid = baseGrid();
    const last = toMin(feeds[feeds.length - 1].f.time);
...
let editingFeedIdx = null;   // index de la prise en cours de modification (ou null)
...
function renderPlan() {
  ...
  slots.forEach(s => {
    if (s.done && s.idx === editingFeedIdx) { list.appendChild(feedEditor(s.idx)); return; }
    ...
    div.innerHTML =
      ...
      '<span class="row" style="gap:8px;align-items:center">' +
        ...
        (s.done ? '<button class="link" data-edit="' + s.idx + '" aria-label="Modifier la prise"><i class="ti ti-pencil"></i></button>' : '') +
      '</span>';
    list.appendChild(div);
  });
  $$('#planList [data-edit]').forEach(b => b.addEventListener("click", () => {
    editingFeedIdx = +b.dataset.edit; renderPlan();
  }));
  const nx = nextSlot();          // pré-remplit « par qui » sur le prochain prévu
  if (nx) setWho(nx.who);
  renderFeedSummary();
}

// Éditeur d'une prise déjà enregistrée (heure, personne, mL, tirage, suppression)
function feedEditor(idx) {
  const f = (state.feeds[dateKey()] || [])[idx] || { time: nowHM(), who: "A", maternel: 0, gallia: 0, tirage: false };
  const wrap = document.createElement("div");
  wrap.className = "slot";
  wrap.style.cssText = "flex-direction:column;align-items:stretch;gap:7px";
  wrap.innerHTML =
    '<div class="row" style="gap:6px">' +
      '<input type="time" class="inp" id="efTime" value="' + f.time + '" style="flex:1">' +
      '<div class="tog" style="flex:1">' +
        '<button type="button" id="efA"' + (f.who === "A" ? ' class="is-on"' : '') + '>Antoine</button>' +
        '<button type="button" id="efC"' + (f.who === "C" ? ' class="is-on"' : '') + '>Célia</button>' +
      '</div>' +
    '</div>' +
    '<div class="row" style="gap:6px">' +
      '<input type="number" class="inp" id="efMat" value="' + (num(f.maternel) || "") + '" placeholder="Maternel mL" style="flex:1" inputmode="numeric">' +
      '<input type="number" class="inp" id="efGal" value="' + (num(f.gallia) || "") + '" placeholder="Gallia mL" style="flex:1" inputmode="numeric">' +
    '</div>' +
    '<label class="check"><input type="checkbox" id="efTir"' + (f.tirage ? ' checked' : '') + '> Tirage de lait</label>' +
    '<div class="row" style="gap:6px">' +
      '<button class="btn ghost" id="efDel" style="flex:1"><i class="ti ti-trash"></i> Supprimer</button>' +
      '<button class="btn ghost" id="efCancel" style="flex:1">Annuler</button>' +
      '<button class="btn" id="efSave" style="flex:1"><i class="ti ti-check"></i> Enregistrer</button>' +
    '</div>';

  let who = f.who;
  wrap.querySelector("#efA").addEventListener("click", () => { who = "A"; wrap.querySelector("#efA").classList.add("is-on"); wrap.querySelector("#efC").classList.remove("is-on"); });
  wrap.querySelector("#efC").addEventListener("click", () => { who = "C"; wrap.querySelector("#efC").classList.add("is-on"); wrap.querySelector("#efA").classList.remove("is-on"); });
  wrap.querySelector("#efCancel").addEventListener("click", () => { editingFeedIdx = null; renderPlan(); });
  wrap.querySelector("#efDel").addEventListener("click", () => {
    (state.feeds[dateKey()] || []).splice(idx, 1);
    editingFeedIdx = null; save(); renderPlan(); scheduleReminder();
  });
  wrap.querySelector("#efSave").addEventListener("click", () => {
    const arr = state.feeds[dateKey()] || (state.feeds[dateKey()] = []);
    if (arr[idx]) {
      arr[idx] = {
        time: wrap.querySelector("#efTime").value || nowHM(),
        who: who,
        maternel: num(wrap.querySelector("#efMat").value),
        gallia: num(wrap.querySelector("#efGal").value),
        tirage: wrap.querySelector("#efTir").checked
      };
    }
    editingFeedIdx = null; save(); renderPlan(); scheduleReminder();
  });
...
function tickTime() {
  const inp = $("#feedTimeInput");
  if (inp && !feedTimeTouched) inp.value = nowHM();   // suit l'heure actuelle tant qu'on n'y touche pas
}


/* ═══════════════════════════════════════════════════════════════════════════
   [7] BIBERONS — SAISIE
   - setWho / getWho : sélection Antoine / Célia
   - tirage : marqueur de tirage de lait
   - validation : enregistre la prise réelle
   ═══════════════════════════════════════════════════════════════════════════ */
...
// Heure de la prise : champ modifiable, pré-rempli à l'heure actuelle.
// (On remplace l'ancien affichage "heure auto" par un vrai champ, sans toucher à index.html.)
let feedTimeTouched = false;
(function setupFeedTimeInput() {
  const span = $("#feedTime"); if (!span) return;
  const wrap = span.parentElement;
  wrap.innerHTML = '<i class="ti ti-clock"></i> heure <input type="time" id="feedTimeInput" class="inp" style="width:auto;padding:3px 6px;font-size:13px;display:inline-block">';
  const inp = $("#feedTimeInput");
  inp.value = nowHM();
  inp.addEventListener("input", () => { feedTimeTouched = true; });
})();

$("#validateFeed").addEventListener("click", () => {
  const d = dateKey();
  if (!state.feeds[d]) state.feeds[d] = [];
  const t = ($("#feedTimeInput") && $("#feedTimeInput").value) || nowHM();
  state.feeds[d].push({
    time: t, who: getWho(),
  ...
  feedTimeTouched = false;                                   // on repasse en "auto"
  const fti = $("#feedTimeInput"); if (fti) fti.value = nowHM();
  renderPlan(); scheduleReminder();
});


/* ═══════════════════════════════════════════════════════════════════════════
   [8] SOINS (bain / pipi / selle)
   Tap = +1 · appui long = −1
   ═══════════════════════════════════════════════════════════════════════════ */
function diapers() { const d = dateKey(); return state.diapers[d] || (state.diapers[d] = { bain: 0, pipi: 0, caca: 0 }); }
...
/* ═══════════════════════════════════════════════════════════════════════════
   [9] PHYSIONOMIE & COURBE
   - ageMonthsAt / renderGrowth : courbe de poids sur abaques
   - renderMeasures / addMeasure : historique des mesures
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ---- To-do list (page Planning) ---- */
function renderTodos() {
  const el = $("#todoList"); if (!el) return;
  const todos = state.todos || [];
  if (!todos.length) { el.innerHTML = '<div class="empty">Aucune tâche.</div>'; return; }
  el.innerHTML = todos.map((t, i) =>
    '<div class="li"><label style="display:flex;align-items:center;gap:9px;flex:1;cursor:pointer">' +
    '<input type="checkbox" data-ti="' + i + '"' + (t.done ? " checked" : "") + ' style="width:19px;height:19px;accent-color:var(--terra);flex:none">' +
    '<span style="' + (t.done ? "text-decoration:line-through;opacity:.55" : "") + '">' + esc(t.text) + '</span>' +
    '</label></div>'
  ).join("");
  $$('#todoList input[type="checkbox"]').forEach(c => c.addEventListener("change", () => {
    state.todos[+c.dataset.ti].done = c.checked; save(); renderTodos();
  }));
}
function addTodo() {
  ...
  state.todos.push({ id: "td" + Date.now(), text: txt, done: false });
  inp.value = ""; save(); renderTodos();
}
(function () {
  const add = $("#addTodo"); if (add) add.addEventListener("click", addTodo);
  const inp = $("#todoInput"); if (inp) inp.addEventListener("keydown", e => { if (e.key === "Enter") { e.preventDefault(); addTodo(); } });
  const clr = $("#clearTodos"); if (clr) clr.addEventListener("click", () => {
    state.todos = (state.todos || []).filter(t => !t.done); save(); renderTodos();
  });
})();
...
function fireReminder(slot) {
  ...
  if ("Notification" in window && Notification.permission === "granted") {
    try { new Notification("Biberon d'" + state.baby.name, { body: slot.time + " · " + Math.round(slot.vol) + " mL · " + who, icon: "icon-192.png" }); } catch (e) { }
  }
...
function buildPrint(range) {
  ...
  let rows = days.map(k => {
    ...
    const dd = state.diapers[k] || { bain: 0, pipi: 0, caca: 0 };
    return "<tr><td>" + frDateFull(k) + "</td><td>" + f.length + "</td><td>" + Math.round(total) + " mL</td><td>" +
      (dd.bain || 0) + " / " + (dd.pipi || 0) + " / " + (dd.caca || 0) + "</td></tr>";
  }).join("");
  ...
  $("#printArea").innerHTML =
    ...
    "<h3>Biberons, bain, pipi & selle</h3>" +
    "<table><tr><th>Jour</th><th>Prises</th><th>Volume</th><th>Bain / pipi / selle</th></tr>" + rows + "</table>" +
...
/* ═══════════════════════════════════════════════════════════════════════════
   [16] RENDU GLOBAL & INIT
   - rerenderAll() : redessine toutes les pages (au démarrage ET quand share.js
                     reçoit une mise à jour distante)
   - window.AgatheApp : pont utilisé par share.js (lire / appliquer l'état)
   - init() : démarrage
   ═══════════════════════════════════════════════════════════════════════════ */
...
// Pont pour le partage (share.js) : lire l'état courant et appliquer un état distant.
window.AgatheApp = {
  getState: () => state,
  applyRemoteState: (d) => {
    if (!d) return;
    state = deepMerge(clone(DEFAULTS), d);
    localStorage.setItem(KEY, JSON.stringify(state)); // pas de save() ici : on évite de re-déclencher le partage
    rerenderAll();
    scheduleReminder();
  },
